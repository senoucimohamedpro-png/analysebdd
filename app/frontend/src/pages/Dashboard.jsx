...
import { getFiches, getFiltres, exportUrl } from "../api/client.js";
...
export default function Dashboard({ analysis }) {
  const [filters, setFilters] = useState({ date_chargement: "", statut: "", code_marketing: "" });
  const [fiches, setFiches] = useState([]);
  const [total, setTotal] = useState(analysis.total_lignes_disponibles);
  const [sortKey, setSortKey] = useState(null);
  const [sortDir, setSortDir] = useState(1);
  const [datesChargement, setDatesChargement] = useState([]);
  const [codesMarketing, setCodesMarketing] = useState([]);

  useEffect(() => {
    getFiltres(analysis.analysis_id).then((r) => {
      setDatesChargement(r.dates_chargement);
      setCodesMarketing(r.codes_marketing);
    });
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="filters-bar">
          ...
          >
            <option value="">Toutes les dates de chargement</option>
            {datesChargement.map((d) => <option key={d} value={d}>{d}</option>)}
          </select>
          <select
